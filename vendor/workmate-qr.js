/* WorkMate Tools local QR renderer. Core QR model: Kazuhiko Arase (MIT), via qrcode-terminal vendor sources. UTF-8 adaptation and canvas renderer added for WorkMate Tools. */
(function(global){
'use strict';
const modules={};
const cache={};
function define(id,fn){modules[id]=fn;}
function req(id){id=String(id).replace(/^\.\//,''); if(cache[id]) return cache[id].exports; if(!modules[id]) throw new Error('QR module not found: '+id); const module={exports:{}}; cache[id]=module; modules[id](module,module.exports,req); return module.exports;}
...
function QR8bitByte(data) {
  ...
  this.parsedData = Array.from(new TextEncoder().encode(this.data));
}
...
const QRCodeModel=req('index');
const QRErrorCorrectLevel=req('QRErrorCorrectLevel');
function renderCanvas(text,size=180,level='M'){
  text=String(text ?? '');
  if(!text) throw new Error('QR Code 內容不可為空白');
  const qr=new QRCodeModel(-1,QRErrorCorrectLevel[level] ?? QRErrorCorrectLevel.M);
  ...
  for(let r=0;r<n;r++) for(let c=0;c<n;c++) if(qr.isDark(r,c)){
    const x1=Math.floor((c+quiet)*size/total), y1=Math.floor((r+quiet)*size/total);
    const x2=Math.ceil((c+quiet+1)*size/total), y2=Math.ceil((r+quiet+1)*size/total);
    ctx.fillRect(x1,y1,x2-x1,y2-y1);
  }
  return canvas;
}
global.OfficeQR={renderCanvas};
})(window);
